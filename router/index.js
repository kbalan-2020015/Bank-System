...
router.post('/adduser', (req, res) => {
    const {userName, userUsername, userAccountNumber, userDPI, userAddress, userMobile, userEmail, userPassword, userWorkName, userMonthlyIncome,
        userAmount} = req.body;
...
router.post('/transfer',(req,res) => {
    const {SenderID, SenderName, SenderAccountNumber, reciverName, reciverAccountNumber, transferAmount} = req.body;
    console.log(transferAmount);
    const history = new historyModel({
        sName: SenderName,
        sAccountNumber: SenderAccountNumber,
        rName: reciverName,
        rAccountNumber:reciverAccountNumber,
        amount: transferAmount
    })

    if(reciverName === 'Select Reciver Name' || reciverAccountNumber === 'Select Reciver Account Number'){
        res.render('sucess',{title: "sucess", value:"", msg: "", errmsg: "All fields are require."});
    }
    else{
        const Sender = customers.find({"_id": SenderID});
        const Reciver = customers.find({"name": reciverName, "account number": reciverAccountNumber});

        Promise.all([Sender,Reciver]).then(([senderData, reciverData]) => {
            senderData.forEach( async (c) => {
                if(c.name === reciverName || c.accountNumber === reciverAccountNumber || c.amount < transferAmount){
